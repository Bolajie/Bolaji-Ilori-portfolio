  ...
  return (
    <div className="pt-32 sm:pt-48 pb-32 bg-slate-950 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="mb-32 text-center animate-reveal">
          ...
          <h1 className="text-5xl sm:text-7xl md:text-9xl font-black text-white mb-10 tracking-tighter leading-none">The <span className="text-gradient">Portfolio</span>.</h1>
          <p className="text-xl sm:text-2xl text-slate-400 max-w-2xl mx-auto font-medium leading-relaxed">
            Scalable automation frameworks and data ecosystems engineered for measurable business growth.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          {CASE_STUDIES.map((study, idx) => (
            <Link 
              ...
              className="group relative flex flex-col bg-slate-900/40 border border-white/5 rounded-[3rem] sm:rounded-[4rem] overflow-hidden hover:border-orange-500/30 transition-all duration-700 animate-reveal hover:scale-[1.01] active:scale-[0.99]"
            ...
            >
              <div className="relative h-[300px] sm:h-[450px] overflow-hidden">
                <img 
                  ...
                  alt={study.title} 
                  className="w-full h-full object-cover grayscale brightness-[0.4] group-hover:grayscale-0 group-hover:scale-105 group-hover:brightness-90 transition-all duration-1000 ease-out"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/20 to-transparent"></div>
                
                <div className="absolute bottom-8 left-8 right-8 sm:bottom-10 sm:left-10 sm:right-10">
                  <div className="flex items-center space-x-3 mb-4">
                    <span className="px-4 py-1.5 bg-white/10 backdrop-blur-md border border-white/10 rounded-full text-[9px] font-black text-white uppercase tracking-widest">
                      {study.category}
                    </span>
                    <span className="w-8 h-px bg-white/20"></span>
                  </div>
                  <h3 className="text-3xl sm:text-4xl md:text-5xl font-black text-white leading-none tracking-tighter group-hover:text-gradient transition-all duration-500">
                    {study.title}
                  </h3>
                </div>
              ...
              <div className="p-8 sm:p-12">
                <div className="grid grid-cols-2 gap-8 mb-10">
                  <div>
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Objective</p>
                    <p className="text-slate-400 text-sm font-medium line-clamp-2 leading-relaxed">{study.problem}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Metric</p>
                    <p className="text-white text-xl font-black">{study.impact.split('and')[0]}</p>
                  </div>
                </div>

                <div className="flex flex-wrap gap-2">
                  {study.tools.slice(0, 4).map(tool => (
                    <span key={tool} className="px-3 py-1.5 bg-white/5 border border-white/5 rounded-xl text-[10px] font-black text-slate-500 uppercase tracking-widest">
                      {tool}
                    </span>
                  ))}
                  {study.tools.length > 4 && (
                    <span className="px-3 py-1.5 text-slate-600 text-[10px] font-black uppercase tracking-widest">+{study.tools.length - 4} More</span>
                  )}
                </div>
              </div>
            </Link>
        ...
        <div className="space-y-48">
          ...
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-24">
            ...
            <div className="lg:col-span-8">
              <p className="text-2xl sm:text-3xl text-slate-400 font-medium leading-relaxed mb-12">
                {study.problem}
              </p>
          ...
          {/* Workflow Gallery (Architecture Visuals) */}
